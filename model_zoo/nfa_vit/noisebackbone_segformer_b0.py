...
class Noise_Attention(Attention):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1, is_last = False, sparse_ratio = 0.25):
    ...
    def forward(self, x, H, W):
        ...
        noise_guided_mask = None
        if self.is_last:
            # Calculate attention masks based on top-k similar values
            # Reshape attention to [B, num_heads, N, N_kv]
            bs, num_heads, num_q, num_k = attn.shape
            
            Topk_thr = max(1, int(num_k * self.sparse_ratio))
            
            topk_values, topk_indices = torch.topk(-1 * attn, Topk_thr, dim=-1)
            
            # Create mask directly using scatter
            mask = torch.zeros_like(attn)
            
            # Flatten the indices for scatter_
            mask = mask.view(bs * num_heads, num_q, num_k)
            mask.scatter_(2, topk_indices.view(bs * num_heads, num_q, Topk_thr), 1.0)
            mask = mask.view(bs, num_heads, num_q, num_k)
...
class Noise_Block(Block):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=GELU, norm_layer=nn.LayerNorm, sr_ratio=1, is_last = False, sparse_ratio = 0.25):
        super().__init__(dim, num_heads, mlp_ratio, qkv_bias, qk_scale, drop, attn_drop, drop_path, act_layer, norm_layer, sr_ratio)
        self.is_last = is_last
        
        self.attn = Noise_Attention(
        dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale,
            attn_drop=attn_drop, proj_drop=drop, sr_ratio=sr_ratio, is_last = is_last, sparse_ratio = sparse_ratio
        )
        
    def forward(self, x, H, W):
        x, noise_guided_mask = self.attn(x, H, W)
...
class MixVisionTransformer_b0(nn.Module):
    def __init__(self, in_chans=3, num_classes=1000, embed_dims=[32, 64, 160, 256],
                 ...
                 depths=[2, 2, 2, 2], sr_ratios=[8, 4, 2, 1], sparse_ratio = 0.25):
        super().__init__()
        ...
        for i in range(depths[0]):
            if i == depths[0] - 1:
                self.block1.append(Noise_Block(
                    dim=embed_dims[0], num_heads=num_heads[0], mlp_ratio=mlp_ratios[0], qkv_bias=qkv_bias, qk_scale=qk_scale,
                    drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i], norm_layer=norm_layer, sr_ratio=sr_ratios[0], is_last = True, sparse_ratio = sparse_ratio
                ))
            else:
                self.block1.append(Noise_Block(
                    dim=embed_dims[0], num_heads=num_heads[0], mlp_ratio=mlp_ratios[0], qkv_bias=qkv_bias, qk_scale=qk_scale,
        ...
        for i in range(depths[1]):  
            if i == depths[1] - 1:
                self.block2.append(Noise_Block(
                    dim=embed_dims[1], num_heads=num_heads[1], mlp_ratio=mlp_ratios[1], qkv_bias=qkv_bias, qk_scale=qk_scale,
                    drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i], norm_layer=norm_layer, sr_ratio=sr_ratios[1], is_last = True, sparse_ratio = sparse_ratio
                ))
            else:
                self.block2.append(Noise_Block(
                    dim=embed_dims[1], num_heads=num_heads[1], mlp_ratio=mlp_ratios[1], qkv_bias=qkv_bias, qk_scale=qk_scale,
        ...
        for i in range(depths[2]):
            if i == depths[2] - 1:
                self.block3.append(Noise_Block(
                    dim=embed_dims[2], num_heads=num_heads[2], mlp_ratio=mlp_ratios[2], qkv_bias=qkv_bias, qk_scale=qk_scale,
                    drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i], norm_layer=norm_layer, sr_ratio=sr_ratios[2], is_last = True, sparse_ratio = sparse_ratio
                ))
            else:
                self.block3.append(Noise_Block(
                    dim=embed_dims[2], num_heads=num_heads[2], mlp_ratio=mlp_ratios[2], qkv_bias=qkv_bias, qk_scale=qk_scale,
        ...
        for i in range(depths[3]):  
            if i == depths[3] - 1:
                self.block4.append(Noise_Block(
                    dim=embed_dims[3], num_heads=num_heads[3], mlp_ratio=mlp_ratios[3], qkv_bias=qkv_bias, qk_scale=qk_scale,
                    drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i], norm_layer=norm_layer, sr_ratio=sr_ratios[3], is_last = True, sparse_ratio = sparse_ratio
                ))
            else:
                self.block4.append(Noise_Block(
                    dim=embed_dims[3], num_heads=num_heads[3], mlp_ratio=mlp_ratios[3], qkv_bias=qkv_bias, qk_scale=qk_scale,
    ...
    def forward(self, x):
        ...
        for i, blk in enumerate(self.block1):
            x, noise_guided_mask = blk.forward(x, H, W)
